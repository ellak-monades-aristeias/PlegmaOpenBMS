...
RangeSelector.prototype = {
	...
	updateButtonStates: function (updating) {
		var rangeSelector = this,
			...
			selected = rangeSelector.selected,
			buttons = rangeSelector.buttons;
        //PLEGMA CUSTOMIZATION

        if(this.chart.options.rangeSelector.updateButtonState==false)
        {
            return;
        }
	...
	setInputValue: function (name, time) {
        var oldValue = this[name + 'Input'].value;
        var options = this.chart.options.rangeSelector;
        if(options.updateButtonState==false)
        {
	...
	drawInput: function (name) {
		...
		this[name + 'DateBox'] = dateBox = renderer.label('', inputGroup.offset)
			.attr({
				padding: 1,
				width: options.inputBoxWidth || 178,
